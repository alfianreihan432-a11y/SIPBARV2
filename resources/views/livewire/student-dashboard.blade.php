        ...
        @if($myRequests->count() > 0)
            ...
            <div class="s-loan-row {{ $st['row'] }}">
                ...
                <div class="s-loan-content">
                    <div class="s-loan-name">{{ $request->item_display_name }}</div>
                    <div class="s-loan-code">Peminjaman #{{ $request->id }} · Qty: {{ $request->totalQuantity() }} unit</div>
                    ...
                    @if(in_array($request->status, ['approved', 'qr_ready']))
                        <div style="margin-top:10px">
                            <button type="button" onclick="openQRModal({{ $request->id }})" class="s-btn s-btn--sm s-btn--primary">
