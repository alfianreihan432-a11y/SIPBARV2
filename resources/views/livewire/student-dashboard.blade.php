<div>
    <style>
        ...
        .db-btn-primary {
            ...
            padding: 8px 16px; background: var(--blue-dark); color: #fff;
            border-radius: 8px; font-size: 12px; font-weight: 600;
            border: none; cursor: pointer; transition: all .2s;
            box-shadow: 0 2px 8px rgba(37,99,235,.25);
        }
        .db-btn-primary:hover { background: var(--blue); transform: translateY(-1px); }
        ...
        .db-modal { background: var(--bg-card); border-radius: 16px; padding: 24px; max-width: 400px; width: calc(100% - 32px); box-shadow: 0 20px 40px rgba(0,0,0,.3); }
        .db-modal-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; }
        .db-modal-title { font-size: 16px; font-weight: 700; color: var(--text-primary); }
        .db-modal-close { width: 32px; height: 32px; border-radius: 8px; background: var(--bg-card-subtle); border: 1px solid var(--border-subtle); display: flex; align-items: center; justify-content: center; color: var(--text-muted); cursor: pointer; transition: all .2s; }
        .db-modal-close:hover { background: var(--bg-hover); color: var(--text-primary); }
        .db-modal-content { text-align: center; }
        .db-modal-qr { margin: 0 auto 16px; border-radius: 12px; overflow: hidden; max-width: 100%; }
        .db-modal-info { font-size: 13px; color: var(--text-muted); margin-bottom: 8px; }
        .db-modal-code { font-family: monospace; font-size: 14px; font-weight: 700; color: var(--text-primary); margin-bottom: 12px; }
        .db-modal-note { font-size: 11px; color: var(--text-subtle); padding: 12px; background: var(--bg-card-subtle); border-radius: 8px; border: 1px solid var(--border-subtle); }
    </style>
    ...
    <div class="db-stat-grid">
        <div class="db-stat-card">
            ...
            <div>
                ...
                <div class="db-stat-change" style="color:#2563eb">Real-time</div>
            </div>
        </div>
        ...
        <div class="db-stat-card">
            <div class="db-stat-icon-box" style="background:rgba(245,158,11,.12)">
            ...
            <div>
                ...
                <div class="db-stat-change" style="color:{{ $pendingRequests > 0 ? '#f59e0b' : '#10b981' }}">
        ...
        @if($myRequests->count() > 0)
            ...
            <div class="db-txn-item">
                <div class="db-txn-top">
                    <div>
                        <div class="db-txn-name">{{ $request->item->name }}</div>
