    ...
    <body class="min-h-screen bg-slate-50 text-slate-900">
        <header class="border-b border-slate-200 bg-white/80 backdrop-blur">
            <div class="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
                ...
                <nav class="flex items-center gap-4 text-sm font-medium">
                    ...
                    @if (Route::has('login'))
                        @auth
                            <a href="{{ route('dashboard') }}" class="rounded-full bg-sky-600 px-4 py-2 text-white">Dashboard</a>
                        @else
                            <a href="{{ route('login') }}" class="rounded-full border border-slate-300 px-4 py-2 text-slate-700">Login</a>
                            @if (Route::has('register'))
                                <a href="{{ route('register') }}" class="rounded-full bg-slate-900 px-4 py-2 text-white">Daftar</a>
                            @endif
                        @endauth
                    @endif
                </nav>
        ...
        <main class="mx-auto max-w-7xl px-6 py-16">
            <section class="grid items-center gap-10 lg:grid-cols-2">
                <div>
                    ...
                    <div class="mt-8 flex flex-wrap gap-4">
                        ...
                        <a href="{{ route('register') }}" class="rounded-full border border-slate-300 px-6 py-3 font-semibold text-slate-700">Daftar Sekarang</a>
                    </div>
                </div>
            ...
            <section id="fitur" class="mt-20 grid gap-6 md:grid-cols-3">
                @foreach ([['title'=>'CRUD Inventaris','desc'=>'Tambah, ubah, hapus, dan telusuri aset sekolah.'],['title'=>'Peminjaman & Pengembalian','desc'=>'Kelola status pinjam, kembali, dan approval.'],['title'=>'Laporan & Export','desc'=>'Siapkan laporan PDF dan Excel dengan cepat.']] as $feature)
                    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                        <h3 class="text-xl font-semibold">{{ $feature['title'] }}</h3>
                        <p class="mt-3 text-slate-600">{{ $feature['desc'] }}</p>
                    </div>
                @endforeach
            </section>
        </main>
        ...
        <footer class="border-t border-slate-200 bg-white/80 py-8">
            <div class="mx-auto max-w-7xl px-6 text-center text-sm text-slate-600">
                © {{ date('Y') }} SIPBAR. Dibuat untuk sekolah modern yang ingin mengelola aset dengan lebih baik.
            </div>
        </footer>
    </body>
</html>
