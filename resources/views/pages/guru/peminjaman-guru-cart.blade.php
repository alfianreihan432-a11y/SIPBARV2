...
@section('title', 'Keranjang Peminjaman Guru')

@section('content')
<div class="max-w-6xl mx-auto px-4 py-6">
    <div class="mb-6 flex items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Keranjang Peminjaman Guru</h1>
            <p class="text-sm text-gray-600 dark:text-gray-300">Pilih beberapa barang lalu review sebelum submit.</p>
        </div>
        <a href="{{ route('teacher.barang') }}" class="inline-flex items-center rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-700">Tambah Barang</a>
    </div>
    ...
    <div class="grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
                ...
                <div class="space-y-3">
                    @foreach($cartItems as $entry)
                        ...
                        <div class="flex items-center justify-between gap-4 rounded-xl border border-slate-200 p-3 dark:border-slate-700">
                            ...
                            <form method="POST" action="{{ route('teacher.peminjaman-guru.cart.remove', $item->id) }}">
                                @csrf
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
            ...
            <form method="POST" action="{{ route('teacher.peminjaman-guru.cart.submit') }}">
                @csrf

                <div class="space-y-4">
                    <div>
                        <label class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-200">Kepala jurusan</label>
                        <select name="kepala_jurusan_id" required class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-800 dark:text-white">
                            <option value="">Pilih kepala jurusan</option>
                            @foreach($kepalaJurusans as $kajur)
                                <option value="{{ $kajur->id }}">{{ $kajur->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <textarea name="purpose" rows="3" required class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-800 dark:text-white" placeholder="Contoh: Praktikum dan kebutuhan pembelajaran..."></textarea>
                    </div>
                    <div>
                ...
                <div class="mt-6 flex gap-3">
                    <a href="{{ route('teacher.peminjaman-guru') }}" class="flex-1 rounded-lg border border-slate-300 px-4 py-2 text-center text-sm font-medium text-slate-700 dark:border-slate-600 dark:text-slate-200">Kembali</a>
