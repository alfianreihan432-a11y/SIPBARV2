    ...
    <div class="content-grid">
        ...
        <div class="content-card">
            ...
            <div class="status-row">
                ...
                <span class="status-count">{{ $approvedRequests }}</span>
            </div>
            ...
            <div class="status-row">
                ...
                <span class="status-count">{{ $rejectedRequests }}</span>
            </div>
        </div>
        ...
        <div class="content-card">
            ...
            <div class="monthly-stat">
                <p class="monthly-value">{{ $thisMonthRequests }}</p>
                <p class="monthly-label">Pengajuan Bulan {{ now()->format('F') }}</p>
            </div>

            <div class="rate-grid">
                <div class="rate-card">
                    <p class="rate-value">{{ round($totalRequests > 0 ? ($completedRequests / $totalRequests) * 100 : 0) }}%</p>
                    <p class="rate-label">Completion Rate</p>
                </div>
                <div class="rate-card">
                    <p class="rate-value">{{ round($totalRequests > 0 ? ($rejectedRequests / $totalRequests) * 100 : 0) }}%</p>
        ...
        <div class="activity-item">
            <div class="activity-left">
                ...
                <div class="activity-info">
                    <p class="activity-name">{{ $activity->user->name ?? 'N/A' }}</p>
                    <p class="activity-detail">{{ $activity->item->name ?? 'N/A' }} • {{ $activity->created_at->diffForHumans() }}</p>
                </div>
            </div>
            <span class="status-badge
                @if($activity->status === 'pending') status-pending
                @elseif($activity->status === 'approved') status-approved
                @elseif($activity->status === 'borrowed') status-borrowed
                @elseif($activity->status === 'returned') status-returned
                @elseif($activity->status === 'rejected') status-rejected
                @endif">
