...
@section('page-heading', 'Riwayat Transaksi')
...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <div class="bg-amber-50 dark:bg-amber-900/20 rounded-lg shadow-sm border border-amber-200 dark:border-amber-800 p-4">
            ...
            <div class="text-2xl font-bold text-amber-900 dark:text-amber-300">{{ $stats['pending'] }}</div>
        </div>
        <div class="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg shadow-sm border border-emerald-200 dark:border-emerald-800 p-4">
            ...
            <div class="text-2xl font-bold text-emerald-900 dark:text-emerald-300">{{ $stats['approved'] }}</div>
        </div>
        <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg shadow-sm border border-blue-200 dark:border-blue-800 p-4">
            ...
            <div class="text-2xl font-bold text-blue-900 dark:text-blue-300">{{ $stats['borrowed'] }}</div>
        </div>
        <div class="bg-gray-50 dark:bg-slate-700 rounded-lg shadow-sm border border-gray-200 dark:border-slate-600 p-4">
            ...
            <div class="text-2xl font-bold text-gray-900 dark:text-gray-300">{{ $stats['returned'] }}</div>
        </div>
        <div class="bg-red-50 dark:bg-red-900/20 rounded-lg shadow-sm border border-red-200 dark:border-red-800 p-4">
            ...
            <div class="text-2xl font-bold text-red-900 dark:text-red-300">{{ $stats['rejected'] }}</div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6">
        ...
        <form method="GET" action="{{ route('transactions.index') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-slate-700">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-semibold text-gray-900 dark:text-white">{{ $transaction->user->name }}</div>
                                <div class="text-xs text-gray-500 dark:text-gray-400">{{ $transaction->user->kelas ?? 'N/A' }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm font-semibold text-gray-900 dark:text-white">{{ $transaction->item->name }}</div>
                            </td>
