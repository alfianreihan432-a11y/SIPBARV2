...
@section('title', 'Verifikasi Pengembalian Barang – SIPBAR Admin')
@section('page-heading', 'Verifikasi Pengembalian')
...
<div>
    ...
    <div class="toolbar-panel">
        <div class="status-filter-pills">
            <a href="{{ route('returns.index', ['status' => 'semua', 'q' => $search]) }}" class="pill-btn {{ $status === 'semua' ? 'active' : '' }}">
                Semua
                <span class="pill-badge">{{ $countSemua }}</span>
            </a>
            <a href="{{ route('returns.index', ['status' => 'menunggu', 'q' => $search]) }}" class="pill-btn {{ $status === 'menunggu' ? 'active' : '' }}">
                Menunggu Verifikasi
                <span class="pill-badge">{{ $countMenunggu }}</span>
            </a>
            <a href="{{ route('returns.index', ['status' => 'disetujui', 'q' => $search]) }}" class="pill-btn {{ $status === 'disetujui' ? 'active' : '' }}">
                Disetujui
                <span class="pill-badge">{{ $countDisetujui }}</span>
            </a>
            <a href="{{ route('returns.index', ['status' => 'ditolak', 'q' => $search]) }}" class="pill-btn {{ $status === 'ditolak' ? 'active' : '' }}">
                Ditolak
                <span class="pill-badge">{{ $countDitolak }}</span>
            </a>
        </div>

        <form method="GET" action="{{ route('returns.index') }}" class="search-form">
            ...
            @if($search)
                <a href="{{ route('returns.index', ['status' => $status]) }}" style="font-size: 11px; color: var(--text-muted); text-decoration: none;">Reset</a>
            @endif
        </form>
    </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @elseif($ret->status === 'ditolak')
                                        ...
                                        @if($ret->alasan_ditolak)
                                            <div style="font-size: 10px; color: #ef4444; margin-top: 3px; max-width: 140px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="{{ $ret->alasan_ditolak }}">
                                                "{{ $ret->alasan_ditolak }}"
                                            </div>
                                        @endif
                                    @endif
                                ...
                                <td style="text-align: right;">
                                    @if($ret->status === 'menunggu')
                                        <div style="display: inline-flex; align-items: center; gap: 6px;">
                                            ...
                                            <button type="button" class="btn-reject" onclick="openRejectModal('{{ $ret->id }}', '{{ addslashes($user?->name ?? 'Siswa') }}', '{{ addslashes($item?->name ?? 'Barang') }}')" title="Tolak Pengembalian">
                                                <svg xmlns="http://www.w3.org/2000/svg" style="width:13px;height:13px" fill="none" viewBox="0 0 24 24" stroke="currentColor">
