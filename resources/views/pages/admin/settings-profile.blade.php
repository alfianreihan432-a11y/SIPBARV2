...
<div style="display:flex;flex-direction:column;gap:20px">
    ...
    {{-- ═══ SETTINGS TABS ═══ --}}
    @include('partials.settings-tabs', ['active' => 'profile'])
    ...
    <div style="background:var(--bg-card);border:1px solid var(--border-alt);border-radius:18px;box-shadow:var(--card-shadow);overflow:hidden">
        ...
        <form method="POST" action="{{ route('settings.profile.update') }}" style="padding:24px 28px;max-width:520px">
            @csrf
            @method('PATCH')
            ...
            <div style="display:flex;flex-direction:column;gap:6px;margin-bottom:18px">
                ...
                <input id="name" name="name" type="text" value="{{ old('name', auth()->user()->name) }}" required autocomplete="name"
                    style="width:100%;background:var(--input-bg);border:1.5px solid {{ $errors->has('name') ? '#f87171' : 'var(--input-border)' }};border-radius:10px;padding:10px 14px;font-size:14px;color:var(--text-primary);outline:none;transition:border-color .2s;font-family:inherit">
                @error('name') <p style="font-size:12px;color:#f87171;margin-top:3px">{{ $message }}</p> @enderror
            </div>
            ...
            <div style="display:flex;flex-direction:column;gap:6px;margin-bottom:24px">
                ...
                <input id="email" name="email" type="email" value="{{ old('email', auth()->user()->email) }}" required autocomplete="email"
                    style="width:100%;background:var(--input-bg);border:1.5px solid {{ $errors->has('email') ? '#f87171' : 'var(--input-border)' }};border-radius:10px;padding:10px 14px;font-size:14px;color:var(--text-primary);outline:none;transition:border-color .2s;font-family:inherit">
