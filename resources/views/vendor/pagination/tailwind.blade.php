    ...
    @if ($paginator->hasPages())
        <nav role="navigation" aria-label="Pagination Navigation" class="sipbar-pagination-container">
            ...
            <div class="sipbar-pagination-controls">
                ...
                @else
                    <a href="{{ $paginator->previousPageUrl() }}" rel="prev" class="sipbar-page-btn" aria-label="{{ __('pagination.previous') }}">
                        <svg class="sipbar-icon-prev" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
                        </svg>
                    </a>
                ...
                @foreach ($elements as $element)
                    ...
                    @if (is_array($element))
                        @foreach ($element as $page => $url)
                            ...
                            @else
                                <a href="{{ $url }}" class="sipbar-page-btn" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                    {{ $page }}
                                </a>
                            @endif
                        @endforeach
                ...
                @if ($paginator->hasMorePages())
                    <a href="{{ $paginator->nextPageUrl() }}" rel="next" class="sipbar-page-btn" aria-label="{{ __('pagination.next') }}">
