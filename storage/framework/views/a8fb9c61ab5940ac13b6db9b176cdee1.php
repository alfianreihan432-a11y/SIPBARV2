<div class="max-w-4xl mx-auto p-4 md:p-6">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg border-2 
        <?php if($action === 'checkout'): ?> border-teal-500 <?php elseif($action === 'checkin'): ?> border-blue-500 <?php else: ?> border-gray-300 dark:border-slate-600 <?php endif; ?>">
        
        
        <div class="p-6 border-b border-gray-200 dark:border-slate-700 
            <?php if($action === 'checkout'): ?> bg-teal-50 dark:bg-teal-900/20 <?php elseif($action === 'checkin'): ?> bg-blue-50 dark:bg-blue-900/20 <?php else: ?> bg-gray-50 dark:bg-slate-700/30 <?php endif; ?>">
            <div class="flex items-start justify-between">
                <div>
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white">Detail Peminjaman</h3>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($action === 'checkout'): ?>
                ...
                <span class="px-3 py-1.5 rounded-lg text-xs font-bold
                    <?php if($transaction->status === 'approved'): ?> bg-emerald-100 text-emerald-800 border border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-300
                    <?php elseif($transaction->status === 'borrowed'): ?> bg-blue-100 text-blue-800 border border-blue-200 dark:bg-blue-900/30 dark:text-blue-300
                    <?php elseif($transaction->status === 'returned'): ?> bg-gray-100 text-gray-800 border border-gray-200 dark:bg-gray-900/30 dark:text-gray-300
                    <?php elseif($transaction->status === 'rejected'): ?> bg-red-100 text-red-800 border border-red-200 dark:bg-red-900/30 dark:text-red-300
                    <?php else: ?> bg-amber-100 text-amber-800 border border-amber-200 dark:bg-amber-900/30 dark:text-amber-300 <?php endif; ?>">
        ...
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <p class="mt-1 font-semibold text-gray-900 dark:text-white"><?php echo e(\Carbon\Carbon::parse($transaction->checkout_at)->format('d M Y H:i')); ?></p>
            </div>
            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
            ...
            <div>
                ...
                <p class="mt-1 font-semibold text-gray-900 dark:text-white"><?php echo e(\Carbon\Carbon::parse($transaction->returned_at)->format('d M Y H:i')); ?></p>
            </div>
            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
        </div>
        ...
        <div class="px-6 pb-6">
            ...
            <p class="mt-1 text-sm text-gray-800 dark:text-gray-200 bg-gray-50 dark:bg-slate-700/50 p-3 rounded-lg"><?php echo e($transaction->purpose); ?></p>
        </div>
        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
        
        
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($action === 'checkin'): ?>
        <div class="px-6 pb-6">
            ...
            <div class="grid grid-cols-3 gap-3 mb-4">
                <label class="relative flex items-center justify-center p-3 border-2 rounded-lg cursor-pointer transition-all
                    <?php echo e($returnCondition === 'good' ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20' : 'border-gray-300 dark:border-slate-600 hover:border-emerald-300'); ?>">
                    <input type="radio" wire:model="returnCondition" value="good" class="sr-only">
                    <div class="text-center">
                        <div class="text-2xl mb-1">✅</div>
                        <div class="text-sm font-semibold <?php echo e($returnCondition === 'good' ? 'text-emerald-700 dark:text-emerald-300' : 'text-gray-700 dark:text-gray-300'); ?>">Baik</div>
                    </div>
                </label>
                <label class="relative flex items-center justify-center p-3 border-2 rounded-lg cursor-pointer transition-all
                    <?php echo e($returnCondition === 'damaged' ? 'border-amber-500 bg-amber-50 dark:bg-amber-900/20' : 'border-gray-300 dark:border-slate-600 hover:border-amber-300'); ?>">
                    <input type="radio" wire:model="returnCondition" value="damaged" class="sr-only">
                    <div class="text-center">
                        <div class="text-2xl mb-1">⚠️</div>
                        <div class="text-sm font-semibold <?php echo e($returnCondition === 'damaged' ? 'text-amber-700 dark:text-amber-300' : 'text-gray-700 dark:text-gray-300'); ?>">Rusak</div>
                    </div>
                </label>
                <label class="relative flex items-center justify-center p-3 border-2 rounded-lg cursor-pointer transition-all
                    <?php echo e($returnCondition === 'lost' ? 'border-red-500 bg-red-50 dark:bg-red-900/20' : 'border-gray-300 dark:border-slate-600 hover:border-red-300'); ?>">
                    <input type="radio" wire:model="returnCondition" value="lost" class="sr-only">
                    <div class="text-center">
                        <div class="text-2xl mb-1">❌</div>
                        <div class="text-sm font-semibold <?php echo e($returnCondition === 'lost' ? 'text-red-700 dark:text-red-300' : 'text-gray-700 dark:text-gray-300'); ?>">Hilang</div>
                    </div>
                </label>
            </div>
...
$message = $__bag->first($__errorArgs[0]); ?>
                <p class="text-sm text-red-600 dark:text-red-400 mb-2"><?php echo e($message); ?></p>
            <?php unset($message);
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
        ...
        <div class="p-6 bg-gray-50 dark:bg-slate-700/30 border-t border-gray-200 dark:border-slate-700 flex gap-3">
            ...
            <button wire:click="cancel" 
                ...
                <?php echo e($action === 'read-only' ? 'Tutup' : 'Batal'); ?>
...
<script>
    ...
    if (!<?php echo json_encode($transaction, 15, 512) ?>) {
        initScanner();
    }
    ...
    ?>
<?php /**PATH C:\Users\ASUS\SIPBARV2\resources\views\livewire\qr-scanner.blade.php ENDPATH**/ ?>